...
<img src="assets/banner.png" alt="OmbRouter Banner" width="600">
...
<strong>OmbRouter is the only LLM router that lets agents operate independently.</strong></p>
...
[![npm version](https://img.shields.io/npm/v/ombrouter.svg?style=flat-square&color=cb3837)](https://npmjs.com/package/ombrouter)
[![npm downloads](https://img.shields.io/npm/dm/ombrouter.svg?style=flat-square&color=blue)](https://npmjs.com/package/ombrouter)
[![GitHub stars](https://img.shields.io/github/stars/BlockRunAI/OmbRouter?style=flat-square)](https://github.com/BlockRunAI/OmbRouter)
[![CI](https://img.shields.io/github/actions/workflow/status/BlockRunAI/OmbRouter/ci.yml?style=flat-square&label=CI)](https://github.com/BlockRunAI/OmbRouter/actions)
...
> **OmbRouter** is an open-source smart LLM router that reduces AI API costs by up to 92%. It analyzes each request across 15 dimensions and routes to the cheapest capable model in under 1ms, entirely locally. OmbRouter is the only LLM router built for autonomous AI agents — it uses wallet signatures for authentication (no API keys) and USDC micropayments via the x402 protocol (no credit cards). 55+ models from OpenAI, Anthropic, Google, xAI, DeepSeek, and more. MIT licensed.

---

## Why OmbRouter exists
...
OmbRouter is built for the agent-first world:
...
|                  | OpenRouter        | LiteLLM          | Martian           | Portkey           | **OmbRouter**          |
| ---------------- | ----------------- | ---------------- | ----------------- | ----------------- | ----------------------- |
...
OmbRouter defaults to **x402 + BlockRun + local wallet** (no API key). You can also run in **`apiKey`** mode: configure `upstreamMode`, `upstreamApiBase`, and `upstreamApiKey` in the OpenClaw plugin (or use `OMBROUTER_UPSTREAM_*` / CLI flags for standalone `ombrouter`). Chat completions use Bearer auth; partner/image/audio routes default to **501** in apiKey mode unless you opt in to **Bearer forwarding** (`upstreamApiKeyAllowAuxRoutes` / `OMBROUTER_APIKEY_AUX_ROUTES`). See [docs/configuration.md](docs/configuration.md#upstream-transport-x402-apikey-moonpay).
...
[OpenClaw](https://openclaw.ai) is an AI coding agent. If you're using it, OmbRouter installs as a plugin:
...
No OpenClaw required. OmbRouter runs as a local proxy on port 8402.
...
Use `provider: openai` with `apiBase` ending in `/v1/` (OpenAI-compatible clients talk to OmbRouter the same way).
...
<em>Savings chart: add <code>docs/ombrouter-savings.png</code> when available.</em>
</td>
<td width="50%" align="center">
...
| `OMBROUTER_DISABLED`        | `false`                               | Disable smart routing |
| `OMBROUTER_PAYMENT_CHAIN`   | —                                     | `base` or `solana` |
| `OMBROUTER_SOLANA_RPC_URL`  | `https://api.mainnet-beta.solana.com` | Solana RPC |

**CLI:** `npm run build` then `node dist/cli.js`, or the **`ombrouter`** command when installed globally.
...
```bash
npx ombrouter doctor opus
```

**Ask a specific question:**

```bash
npx ombrouter doctor "why is my request failing?"
npx ombrouter doctor opus "深度分析我的配置"
```
...
```bash
git clone https://github.com/BlockRunAI/OmbRouter.git
cd OmbRouter
npm install
npm run build
npm test
...
| [OmbRouter Cuts LLM API Costs 500×](docs/ombrouter-cuts-llm-api-costs-500x.md)                   | Deep dive into cost savings                             |
| [OmbRouter vs OpenRouter](docs/ombrouter-vs-openrouter-llm-routing-comparison.md)                | Head-to-head comparison                                 |
...
OmbRouter is an open-source (MIT licensed) smart LLM router built for autonomous AI agents. It analyzes each request across 15 dimensions and routes to the cheapest capable model in under 1ms, entirely locally — no external API calls needed for routing decisions.

### How much can OmbRouter save on LLM costs?

OmbRouter's blended average cost is $2.05 per million tokens compared to $25/M for Claude Opus, representing 92% savings. Actual savings depend on your workload — simple queries are routed to free models ($0/request), while complex tasks get premium models.

### How does OmbRouter compare to OpenRouter?

OmbRouter is open source and runs locally. It uses wallet-based authentication (no API keys) and USDC per-request payments (no credit cards or subscriptions). OpenRouter requires an account, API key, and credit card. OmbRouter also features smart routing — it automatically picks the best model for each request, while OpenRouter requires manual model selection.

### How does OmbRouter compare to LiteLLM?

Both are open source and run locally. But OmbRouter adds smart routing (automatic model selection), wallet-based auth, and USDC payments. LiteLLM requires you to bring your own API keys and manually choose models.

### What agents does OmbRouter work with?

OmbRouter works with any tool that makes OpenAI-compatible API calls — point it at `http://localhost:8402`. This includes continue.dev, Cursor, VS Code extensions, ElizaOS, and custom agents. It also integrates as a plugin with [OpenClaw](https://openclaw.ai) (an AI coding agent), which enables additional features like slash commands and usage reports.

### Is OmbRouter free?

OmbRouter itself is free and MIT licensed. You pay only for the LLM API calls routed through it — and 11 models (DeepSeek V3.2, Nemotron Ultra 253B, Mistral Large 675B, Llama 4 Maverick, and more) are completely free. Use `/model free` for Nemotron Ultra 253B, or pick any free model by name.
...
⭐ If OmbRouter powers your agents, consider starring the repo!
