# OmbRouter Update Script for Windows (PowerShell)
# Usage: iwr -useb https://blockrun.ai/OmbRouter-update.ps1 | iex
#    or: powershell -ExecutionPolicy Bypass -Command "iwr -useb https://blockrun.ai/OmbRouter-update.ps1 | iex"
#
# Run as regular user (no admin needed)
Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"

$PLUGIN_DIR = "$env:USERPROFILE\.openclaw\extensions\ombrouter"
...
Write-Host ""
Write-Host "OmbRouter Update (Windows)" -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path $CONFIG_PATH) {
    try {
        ...
        $changed = $false
        if ($cfg.plugins -and $cfg.plugins.entries -and $cfg.plugins.entries.PSObject.Properties['ombrouter']) {
            $cfg.plugins.entries.PSObject.Properties.Remove('ombrouter')
            $changed = $true
        }
        if ($cfg.plugins -and $cfg.plugins.installs -and $cfg.plugins.installs.PSObject.Properties['ombrouter']) {
            $cfg.plugins.installs.PSObject.Properties.Remove('ombrouter')
            $changed = $true
        }
        if ($changed) {
...
try {
    $verFile = Join-Path $env:TEMP "ombrouter-latest-ver.txt"
    & cmd /c "npm view ombrouter@latest version > `"$verFile`" 2>nul"
...
Write-Step "Downloading OmbRouter v$LATEST_VERSION from npm..."

$tmpDir = Join-Path $env:TEMP "ombrouter-install-$(Get-Random)"
New-Item -ItemType Directory -Path $tmpDir -Force | Out-Null

try {
    # Run npm pack via cmd.exe to completely bypass PowerShell's stderr-as-error behavior
    & cmd /c "npm pack `"ombrouter@$LATEST_VERSION`" --pack-destination `"$tmpDir`" --prefer-online >nul 2>nul"
...
$logFile = Join-Path $env:TEMP "ombrouter-npm-install.log"
...
if (Test-Path $CONFIG_PATH) {
    try {
        ...
        if (-not $allow.Contains('ombrouter')) {
            $allow.Add('ombrouter')
            $cfg.plugins.allow = $allow.ToArray()
            $cfg | ConvertTo-Json -Depth 20 | Set-Content $CONFIG_PATH -Encoding UTF8
            Write-Ok "Added ombrouter to plugins.allow"
        } else {
...
# ── Done ──────────────────────────────────────────────────────
Write-Host ""
Write-Host "OmbRouter v$LATEST_VERSION installed successfully!" -ForegroundColor Green
Write-Host ""
Write-Host "  Run: openclaw gateway restart" -ForegroundColor Cyan
Write-Host ""
Write-Host "  To verify: npx ombrouter doctor"
Write-Host ""
