...
restore_previous_install() {
  ...
  if [ "$exit_code" -ne 0 ]; then
    echo ""
    echo "✗ Update failed. Restoring previous OmbRouter install..."
...
run_dependency_install() {
  local plugin_dir="$1"
  local log_file="$HOME/ombrouter-npm-install.log"
...
if [ -f "$WALLET_FILE" ]; then
  ...
  if [[ "$WALLET_KEY" == 0x* ]] && [ "$KEY_LEN" -eq 66 ]; then
    ...
    WALLET_ADDRESS=$(node -e "
      try {
        const { privateKeyToAccount } = require('$HOME/.openclaw/extensions/ombrouter/node_modules/viem/accounts/index.js');
...
if [ -f "$CONFIG_PATH" ]; then
  CONFIG_BACKUP="$CONFIG_PATH.ombrouter-update.$(date +%s).bak"
...
# Remove it so OmbRouter owns /wallet without conflict.
...
try {
  ...
  // Clean plugins.allow — remove ombrouter (re-added later) and any stale bare
  ...
  if (Array.isArray(config?.plugins?.allow)) {
    ...
    config.plugins.allow = config.plugins.allow.filter(p => {
      if (p === 'ombrouter' || p === 'ombrouter') return false;
...
echo "→ Installing latest OmbRouter..."
...
# openclaw plugins install writes plugins.entries.OmbRouter (PascalCase, from
# plugin name) AND plugins.installs.ombrouter (lowercase, from plugin id).
...
# ── Summary ─────────────────────────────────────────────────────
echo ""
echo "✓ OmbRouter updated successfully!"
echo ""
