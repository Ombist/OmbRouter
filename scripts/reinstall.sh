...
PLUGIN_DIR="$HOME/.openclaw/extensions/ombrouter"
...
restore_previous_install() {
  ...
  if [ "$exit_code" -ne 0 ]; then
    echo ""
    echo "✗ Reinstall failed. Restoring previous OmbRouter install..."
...
run_dependency_install() {
  local plugin_dir="$1"
  local log_file
  log_file="$(mktemp -t ombrouter-reinstall-npm.XXXXXX.log)"
...
if [ -d "$PLUGIN_DIR" ]; then
  PLUGIN_BACKUP="$HOME/.openclaw/blockrun/ombrouter.backup.$(date +%s)"
...
if [ -f "$CONFIG_PATH" ]; then
  CONFIG_BACKUP="$CONFIG_PATH.ombrouter-reinstall.$(date +%s).bak"
...
# lobster.cash conflicts with /wallet command — remove it so OmbRouter owns /wallet.
...
for (const key of ['ombrouter', 'OmbRouter', 'ombrouter']) {
  if (c.plugins?.entries?.[key]) delete c.plugins.entries[key];
  if (c.plugins?.installs?.[key]) delete c.plugins.installs[key];
}

// Clean plugins.allow — remove ombrouter (will be re-added after install)
...
if (Array.isArray(c.plugins?.allow)) {
  ...
  c.plugins.allow = c.plugins.allow.filter(p => {
    if (p === 'ombrouter' || p === 'ombrouter') return false; // re-added later
...
for stale in "$HOME/.openclaw/extensions/ombrouter.backup."* "$HOME/.openclaw/extensions/.openclaw-install-stage-"*; do
  [ -d "$stale" ] && rm -rf "$stale"
done

echo "→ Installing OmbRouter..."
# Run with timeout — openclaw plugins install may hang after printing
# "Installed plugin: ombrouter" in OpenClaw v2026.4.5 (parallel plugin loading).
# 120s is enough for slow connections; the install itself completes in ~30s.
if command -v timeout >/dev/null 2>&1; then
  timeout 120 openclaw plugins install ombrouter || {
    exit_code=$?
...
force_install_from_npm() {
  ...
  local TMPPACK
  TMPPACK=$(mktemp -d)
  if npm pack "ombrouter@${version}" --pack-destination "$TMPPACK" --prefer-online >/dev/null 2>&1; then
    local TARBALL
    TARBALL=$(ls "$TMPPACK"/blockrun-ombrouter-*.tgz 2>/dev/null | head -1)
    if [ -n "$TARBALL" ]; then
      rm -rf "$PLUGIN_DIR"
      mkdir -p "$PLUGIN_DIR"
...
if [ ! -f "$DIST_PATH" ]; then
  ...
  LATEST_VER=$(npm view ombrouter@latest version 2>/dev/null || echo "")
  ...
  if [ ! -f "$DIST_PATH" ]; then
    ...
    echo "  Please report this issue at https://github.com/BlockRunAI/OmbRouter/issues"
    exit 1
  fi
else
...
INSTALLED_VER=$(node -e "try{const p=require('$PLUGIN_DIR/package.json');console.log(p.version);}catch{console.log('?');}" 2>/dev/null || echo "?")
echo "  ✓ OmbRouter v${INSTALLED_VER} installed"
...
if (fs.existsSync(configPath)) {
  try {
    ...
    // Ensure plugins.allow exists and includes ombrouter
    if (!config.plugins) config.plugins = {};
    if (!Array.isArray(config.plugins.allow)) {
      config.plugins.allow = [];
    }
    if (!config.plugins.allow.includes('ombrouter') && !config.plugins.allow.includes('ombrouter')) {
      config.plugins.allow.push('ombrouter');
      console.log('  Added ombrouter to plugins.allow');
    } else {
...
CLEANED=0
for backup_dir in "$HOME/.openclaw/extensions/ombrouter.backup."* "$HOME/.openclaw/blockrun/ombrouter.backup."*; do
...
try {
  const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
  let changed = false;
  const isStale = (p) => p.includes('.openclaw-install-stage-') || p.includes('ombrouter.backup.');
...
if systemctl --user is-active openclaw-gateway.service >/dev/null 2>&1 || \
  ...
  if systemctl --user restart openclaw-gateway.service 2>/dev/null; then
    # Wait up to 15s for OmbRouter proxy port to come up
    ...
    if $RESTART_OK; then
      echo "  ✓ Gateway restarted — OmbRouter active on port 8402"
    else
...
echo ""
echo "CLI commands:"
echo "  npx ombrouter report            # daily usage report"
echo "  npx ombrouter report weekly      # weekly report"
echo "  npx ombrouter report monthly     # monthly report"
echo "  npx ombrouter doctor             # AI diagnostics"
echo ""
echo "To uninstall: bash ~/.openclaw/extensions/ombrouter/scripts/uninstall.sh"
