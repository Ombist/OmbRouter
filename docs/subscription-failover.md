# Using Subscriptions with OmbRouter Failover

This guide explains how to use your existing LLM subscriptions (Claude Pro/Max, ChatGPT Plus, etc.) as primary providers, with OmbRouter x402 micropayments as automatic failover.

## Why Not Built Into OmbRouter?

After careful consideration, we decided **not** to integrate subscription support directly into OmbRouter for several important reasons:
...
- We want to keep OmbRouter compliant and low-risk for all users

### 2. Security & Privacy

- Integrating subscriptions would require OmbRouter to access your credentials/sessions
...
- Duplicating this in OmbRouter would increase complexity without added value

### 4. Better Architecture

- OpenClaw's native failover mechanism is more flexible and powerful
- Works with **any** provider (not just Claude)
- Zero code changes needed in OmbRouter
...
- ✅ OmbRouter stays focused on cost optimization

## Setup Guide

### Prerequisites

1. **OpenClaw Gateway installed** with OmbRouter plugin
...
3. **OmbRouter wallet funded** (for failover)
   ```bash
...
#### Step 2: Add OmbRouter as Fallback
...
2. **Make a request** - OpenClaw should automatically failover to OmbRouter
3. **Check logs:**
   ```bash
...
#### Tier-Based Configuration (OmbRouter Smart Routing)

When using `blockrun/auto`, OmbRouter automatically classifies your request and picks the cheapest capable model:
...
Learn more: [OmbRouter Smart Routing](./smart-routing.md)
...
# Check OmbRouter proxy logs
openclaw gateway logs | grep "OmbRouter"
```
...
- ✅ "OmbRouter: Success with model" → failover succeeded

### Common Issues

#### Issue: Failover never triggers

**Symptoms:** Always uses primary, never switches to OmbRouter
...
**Symptoms:** Failover triggers but OmbRouter returns balance errors

**Solutions:**

1. Check OmbRouter wallet balance:
   ```bash
...
**Symptoms:** 5-10 second delay when switching to OmbRouter
...
- OmbRouter x402: ~$5/month (50 requests via smart routing)
...
**A:** You configure the subscription directly in OpenClaw using your own credentials. OmbRouter only receives requests after your subscription fails. This is similar to using multiple API keys yourself.
...
**A:** PR #15 integrated Claude CLI directly into OmbRouter. Our approach:

- ✅ Works with any provider (not just Claude)
- ✅ Respects provider ToS (no proxy/wrapper)
- ✅ Uses OpenClaw's native failover (more reliable)
- ✅ Zero maintenance burden on OmbRouter

## Feedback & Support

We'd love to hear your experience with subscription failover:

- **GitHub Discussion:** [Share your setup](https://github.com/BlockRunAI/OmbRouter/discussions)
- **Issues:** [Report problems](https://github.com/BlockRunAI/OmbRouter/issues)
...
- [OmbRouter Smart Routing](./smart-routing.md)
- [OmbRouter x402 Micropayments](./x402-payments.md)
