...
**Goal:** Let OmbRouter users opt in as worker nodes that execute HTTP health checks and earn USDC micropayments via x402.

**Architecture:** OmbRouter polls BlockRun for tasks every 30s, executes HTTP checks, signs results with its existing wallet key, and submits to BlockRun. BlockRun verifies the signature, accumulates credits per worker, and pays out via x402 (TransferWithAuthorization with `payTo = worker address`) when credits hit a $0.01 threshold.
...
Workers are existing paying OmbRouter users. The actual work (one HTTP fetch) is cheaper than writing cheating code. Reward is $0.0001 — no rational incentive to fabricate. Simple signature proves identity; that's enough.
...
## Task 4: OmbRouter — Wire Worker Mode in index.ts
...
```typescript
// Worker mode: opt-in via OMBROUTER_WORKER=1 or --worker flag
...
git commit -m "feat(worker): activate WorkerNode when OMBROUTER_WORKER=1"
```
...
async function sendUsdcToWorker(
  ...
  // Same scheme as OmbRouter's x402.ts but payTo = worker address
...
**OmbRouter** (no new env vars needed for basic mode):

```bash
OMBROUTER_WORKER=1        # opt-in to worker mode
...
**Step 3:** Start OmbRouter in worker mode (pointing at localhost)
...
- `/wallet worker-status` command in OmbRouter to show earnings
