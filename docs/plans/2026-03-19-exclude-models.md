...
**Tech Stack:** TypeScript, Node.js fs, existing OmbRouter command pattern
...
const TEST_DIR = join(tmpdir(), "ombrouter-test-exclude-" + Date.now());
...
if (excludeExcluded.length > 0) {
  console.log(
    `[OmbRouter] Exclude filter: excluded ${excludeExcluded.join(", ")} (user preference)`,
  );
}
```
