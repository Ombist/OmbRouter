...
OmbRouter runs a local proxy on port `8402` that handles x402 payments automatically. Point any OpenAI-compatible client at it:
...
OpenAI-compatible endpoint. Route via OmbRouter proxy (`http://localhost:8402`) for automatic x402 payment handling.
...
Edit an existing image using AI. Route via OmbRouter proxy (`http://localhost:8402`) for automatic x402 payment handling.
...
| ------------------ | ---------------------------------- | ---------------------------------------------- |
| Local file path    | `"/Users/me/photo.png"`            | Absolute path — OmbRouter reads the file      |
| Home-relative path | `"~/photo.png"`                    | Expands `~` to home directory                  |
| HTTP/HTTPS URL     | `"https://example.com/photo.png"`  | OmbRouter downloads the image automatically   |
...
If you're using OmbRouter as a library:
...
```typescript
// OmbRouter reads the file for you — no base64 encoding needed
...
When using OmbRouter with OpenClaw, generate and edit images directly from any conversation:
...
- **Image editing** — The `/v1/images/image2image` endpoint currently supports `openai/gpt-image-1` (default). The `image` and `mask` fields accept local file paths (`~/photo.png`, `/abs/path.png`), HTTP/HTTPS URLs, or base64 data URIs. OmbRouter handles file reading and URL downloading automatically. Supported formats: PNG, JPG/JPEG, WebP.
