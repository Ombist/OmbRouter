...
Through [OmbRouter](https://github.com/blockrunai/OmbRouter) — BlockRun's local AI routing proxy — you get zero-cost access to the following:
...
OmbRouter's value proposition isn't just "here are free models." It's **intelligent routing** — automatically selecting the right model for each request based on prompt complexity.

### The Four-Tier Architecture

OmbRouter classifies every incoming request into one of four complexity tiers:
...
| Pure GPT-4o                 | ~$15–30                |
| OmbRouter AUTO mode        | ~$5–10                 |
| OmbRouter ECO mode         | ~$1–3                  |
...
GPT-OSS is BlockRun's default general-purpose free model. The 120B version is OmbRouter's **default SIMPLE-tier model** in ECO mode and the **ultimate fallback** when wallet balance runs low. It handles conversation, text generation, and summarization with reliable consistency.
...
253 billion parameters. Reasoning capability. 131K context window. Nemotron Ultra is the **single strongest free model on BlockRun** — and it's the default when you type `/model free` in OmbRouter.
...
DeepSeek has consistently punched above its weight on coding benchmarks. V3.2 adds reasoning capability on top of already strong code generation. It's OmbRouter's **MEDIUM-tier primary in ECO mode** — the model that handles your everyday coding tasks for free.
...
### Option 1: Via OmbRouter (Recommended)
...
Seven models are marked reasoning-capable, and they handle most tasks well. But on the hardest problems — competition-level math, formal proofs, deep multi-step planning — they don't match Claude Opus 4 or o3. That's why OmbRouter's REASONING tier doesn't use free models.
...
BlockRun and OmbRouter exist to be the **routing layer** in this transition: not locked to any single provider, not bound to any single model, always giving developers the lowest-cost path to the right capability.
...
_Based on OmbRouter v0.12.84. Model availability may change with future releases. For the latest information, visit [blockrun.ai](https://blockrun.ai)._
