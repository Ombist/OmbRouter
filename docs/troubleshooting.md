...
Quick solutions for common OmbRouter issues.

> Need help? [Open a Discussion](https://github.com/BlockRunAI/OmbRouter/discussions) or check [existing issues](https://github.com/BlockRunAI/OmbRouter/issues).
...
cat ~/.openclaw/extensions/ombrouter/package.json | grep version
...
### "Config validation failed: plugin not found: ombrouter"
...
Wallet needs funding. OmbRouter accepts **USDC** (not SOL or ETH) on either chain.
...
This is a **false positive**. OmbRouter legitimately:
...
OpenClaw's security scanner may flag OmbRouter with:
...
OmbRouter reads `BLOCKRUN_WALLET_KEY` to sign x402 payment transactions — this is required and intentional:
...
- Source code is [MIT licensed and fully auditable](https://github.com/BlockRunAI/OmbRouter)
...
As of v0.4.1, OmbRouter automatically detects and reuses an existing proxy on the configured port instead of failing with `EADDRINUSE`. You should no longer see this error.
...
```bash
curl -fsSL https://raw.githubusercontent.com/BlockRunAI/OmbRouter/main/scripts/reinstall.sh | bash
openclaw gateway restart
```
