---
name: release
description: Use this skill for EVERY OmbRouter release. Enforces the full checklist — version sync, CHANGELOG, blockrun server constant, build, tests, npm publish, git tag, GitHub release. No step can be skipped.
---

# OmbRouter Release Checklist
...
## Step 4: Sync `CURRENT_OMBROUTER_VERSION` in blockrun
...
```typescript
const CURRENT_OMBROUTER_VERSION = "x.y.z";
```

Update it to match the new version. Verify with:

```bash
grep CURRENT_OMBROUTER_VERSION /Users/vickyfu/Documents/blockrun-web/blockrun/src/app/api/v1/chat/completions/route.ts
```
...
Verify the release on GitHub: https://github.com/BlockRunAI/OmbRouter/releases
...
Verify: https://npmjs.com/package/ombrouter

Expected output: `+ ombrouter@{VERSION}`
...
# 3. blockrun CURRENT_OMBROUTER_VERSION
grep CURRENT_OMBROUTER_VERSION /Users/vickyfu/Documents/blockrun-web/blockrun/src/app/api/v1/chat/completions/route.ts

# 4. npm package is live
npm view ombrouter version
...
| Mistake                                                   | Prevention                            |
| --------------------------------------------------------- | ------------------------------------- |
| Forgot to update `CURRENT_OMBROUTER_VERSION` in blockrun | Step 4 — always check                 |
