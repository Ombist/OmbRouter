...
Generate or edit images through OmbRouter. Payment is automatic via x402.

**Dual proxy:** Image routes must hit the **x402** (or **moonpay**) OmbRouter instance — examples use `http://localhost:8402`. If your paid-auxiliary proxy listens on another host/port, substitute it. See [dual-proxy-deployment.md](../../docs/dual-proxy-deployment.md).
...
OmbRouter automatically downloads URLs and reads local file paths — pass them directly, no manual base64 conversion needed.
...
- Google models may return base64 internally — OmbRouter uploads automatically and returns a hosted URL
