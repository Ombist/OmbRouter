...
- **Predexon skill** — New vendor skill ships with OmbRouter: 39 prediction market endpoints (Polymarket, Kalshi, dFlow, Binance, cross-market matching, wallet analytics, smart money). OpenClaw agents now auto-invoke this skill when users ask about prediction markets, market odds, or smart money positioning.
- **Partner proxy extended** — `/v1/pm/*` paths now route through OmbRouter's partner proxy (same as `/v1/x/*`), enabling automatic x402 payment for all Predexon endpoints via `localhost:8402`.
...
- **Skills not found by OpenClaw agents** — Auto-copies bundled skills (imagegen, x-api, ombrouter) to `~/.openclaw/workspace/skills/` on plugin registration. Fixes `ENOENT` errors when agents invoke `/imagegen`.
...
- **Internal `release` skill excluded** — The release checklist skill is for OmbRouter maintainers only and is no longer installed to user workspaces.
...
- **Comprehensive docs refresh** — architecture updated for dual-chain (Base + Solana), configuration updated with all env vars (`OMBROUTER_SOLANA_RPC_URL`, `OMBROUTER_WORKER`), troubleshooting updated for USDC-on-Solana funding, CHANGELOG backfilled for v0.11.14–v0.12.24
...
- **Update nudge improved** — now shows `npx ombrouter@latest` instead of `curl | bash`
...
- **Server-side update nudge** — 429 responses from BlockRun now surface update hints when running an outdated OmbRouter version
...
- **`ombrouter report` command** — daily/weekly/monthly usage reports via `npx ombrouter report`
- **`ombrouter doctor` command** — AI diagnostics for troubleshooting
