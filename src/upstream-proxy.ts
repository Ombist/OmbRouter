 ...
 *
 * Usage:
 *   BLOCKRUN_UPSTREAM_PROXY=socks5://127.0.0.1:1080 ombrouter start
 *   BLOCKRUN_UPSTREAM_PROXY=http://127.0.0.1:8080 ombrouter start
 */
...
export async function applyUpstreamProxy(proxyUrl?: string): Promise<string | undefined> {
  ...
  try {
    ...
    console.warn(`[OmbRouter] Invalid BLOCKRUN_UPSTREAM_PROXY URL: ${url} — skipping proxy setup`);
    return undefined;
  }
  ...
  try {
    if (scheme === "socks5:" || scheme === "socks4:") {
      ...
      console.warn(
        `[OmbRouter] Unsupported proxy scheme "${scheme}" in BLOCKRUN_UPSTREAM_PROXY — use http:// or socks5://`,
      );
      return undefined;
    }
  } catch (err) {
    console.warn(
      `[OmbRouter] Failed to configure upstream proxy "${url}": ${err instanceof Error ? err.message : err}`,
    );
    return undefined;
  }
