/**
 * OmbRouter Doctor - AI-Powered Diagnostics
 *
...
interface NetworkInfo {
  ...
  localProxy: { running: boolean; port: number; upstreamMode?: string };
}
...
/** Do not query the public npm registry for latest version. */
...
async function collectNetworkInfo(): Promise<NetworkInfo> {
  ...
  let proxyUpstreamMode: string | undefined;
  try {
    ...
    if (response.ok) {
      proxyRunning = true;
      try {
        const data = (await response.json()) as { upstreamMode?: string };
        if (data.upstreamMode) proxyUpstreamMode = data.upstreamMode;
      } catch {
        /* ignore */
      }
    }
  ...
  return {
    ...
    localProxy: { running: proxyRunning, port, upstreamMode: proxyUpstreamMode },
  };
}
...
function identifyIssues(result: DiagnosticResult): string[] {
  const issues: string[] = [];
  const noLocalWalletProxy =
    result.network.localProxy.upstreamMode === "apiKey" ||
    result.network.localProxy.upstreamMode === "moonpay";

  if (!noLocalWalletProxy && !result.wallet.exists) {
    issues.push("No wallet found");
  }
  if (!noLocalWalletProxy && result.wallet.isEmpty) {
  ...
  } else if (!noLocalWalletProxy && result.wallet.isLow) {
...
function printDiagnostics(result: DiagnosticResult): void {
  ...
  // Version (no remote npm "latest" check)
  console.log("Version");
  ...
  // Wallet
  console.log("\nWallet");
  if (noLocalWalletProxy) {
    console.log(
      `  ${yellow("Local proxy: apiKey or MoonPay upstream — OmbRouter file wallet not required for chat via this proxy.")}`,
    );
  }
  ...
  } else if (!noLocalWalletProxy) {
    console.log(`  ${red("No wallet found")}`);
  } else {
    console.log(`  ${yellow("No local wallet file (optional for apiKey / MoonPay upstream mode)")}`);
  }
  ...
  if (result.network.localProxy.running) {
    const modeHint = result.network.localProxy.upstreamMode
      ? ` (${result.network.localProxy.upstreamMode})`
      : "";
    console.log(
      `  ${green(`Local proxy: running on :${result.network.localProxy.port}${modeHint}`)}`,
    );
  } else {
...
async function analyzeWithAI(
  ...
  const doctorNoLocalWallet =
    diagnostics.network.localProxy.upstreamMode === "apiKey" ||
    diagnostics.network.localProxy.upstreamMode === "moonpay";
  if (!doctorNoLocalWallet && diagnostics.wallet.isEmpty) {
    ...
    const response = await paymentFetch("https://blockrun.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a technical support expert for BlockRun and OmbRouter.
