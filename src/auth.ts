 ...
 * - Without the key, OmbRouter cannot authorize payments to BlockRun
...
async function loadSavedWallet(): Promise<string | undefined> {
  try {
    ...
    if (key.startsWith("0x") && key.length === 66) {
      console.log(`[OmbRouter] ✓ Loaded existing wallet from ${WALLET_FILE}`);
      return key;
    }
    ...
    console.error(`[OmbRouter] ✗ CRITICAL: Wallet file exists but has invalid format!`);
    console.error(`[OmbRouter]   File: ${WALLET_FILE}`);
    console.error(`[OmbRouter]   Expected: 0x followed by 64 hex characters (66 chars total)`);
    console.error(
      `[OmbRouter]   To fix: restore your backup key or set BLOCKRUN_WALLET_KEY env var`,
    );
    throw new Error(
    ...
    if ((err as NodeJS.ErrnoException).code !== "ENOENT") {
      ...
      console.error(
        `[OmbRouter] ✗ Failed to read wallet file: ${err instanceof Error ? err.message : String(err)}`,
      );
      throw new Error(
...
async function loadMnemonic(): Promise<string | undefined> {
  try {
    ...
    console.warn(`[OmbRouter] ⚠ Mnemonic file exists but has invalid format — ignoring`);
    return undefined;
  } catch (err) {
    // Only swallow ENOENT (file not found)
    if ((err as NodeJS.ErrnoException).code !== "ENOENT") {
      console.warn(`[OmbRouter] ⚠ Cannot read mnemonic file — ignoring`);
    }
  }
  return undefined;
}
...
async function generateAndSaveWallet(): Promise<{
  ...
  if (existingMnemonic) {
    throw new Error(
      `Mnemonic file exists at ${MNEMONIC_FILE} but wallet.key is missing.\n` +
        ...
        `Then run: ombrouter (or: node dist/cli.js)`,
    );
  }
  ...
  try {
    ...
    console.log(`[OmbRouter] Wallet saved and verified at ${WALLET_FILE}`);
  } catch (err) {
    throw new Error(
  ...
  console.log(`[OmbRouter]`);
  console.log(`[OmbRouter] ════════════════════════════════════════════════`);
  console.log(`[OmbRouter]   NEW WALLET GENERATED — BACK UP YOUR KEY NOW`);
  console.log(`[OmbRouter] ════════════════════════════════════════════════`);
  console.log(`[OmbRouter]   EVM Address    : ${derived.evmAddress}`);
  if (solanaAddress) {
    console.log(`[OmbRouter]   Solana Address : ${solanaAddress}`);
  }
  console.log(`[OmbRouter]   Key file       : ${WALLET_FILE}`);
  console.log(`[OmbRouter]   Mnemonic       : ${MNEMONIC_FILE}`);
  console.log(`[OmbRouter]`);
  console.log(`[OmbRouter]   Both EVM (Base) and Solana wallets are ready.`);
  console.log(`[OmbRouter]   To back up, run in OpenClaw:`);
  console.log(`[OmbRouter]     /wallet export`);
  console.log(`[OmbRouter]`);
  console.log(`[OmbRouter]   To restore on another machine:`);
  console.log(`[OmbRouter]     export BLOCKRUN_WALLET_KEY=<your_key>`);
  console.log(`[OmbRouter] ════════════════════════════════════════════════`);
  console.log(`[OmbRouter]`);
 ...
 *
 * ONLY works when the mnemonic was originally generated by OmbRouter
...
export async function recoverWalletFromMnemonic(): Promise<void> {
  const mnemonic = await loadMnemonic();
  if (!mnemonic) {
    console.error(`[OmbRouter] No mnemonic found at ${MNEMONIC_FILE}`);
    console.error(`[OmbRouter] Cannot recover — no mnemonic to derive from.`);
    process.exit(1);
  }
  ...
  if (existing) {
    console.error(`[OmbRouter] wallet.key already exists at ${WALLET_FILE}`);
    console.error(`[OmbRouter] Recovery not needed.`);
    process.exit(1);
  }
  ...
  console.log(`[OmbRouter]`);
  console.log(`[OmbRouter] ⚠  WALLET RECOVERY FROM MNEMONIC`);
  console.log(`[OmbRouter] ════════════════════════════════════════════════`);
  console.log(`[OmbRouter]   This only works if your mnemonic was originally`);
  console.log(`[OmbRouter]   generated by OmbRouter (not set manually).`);
  console.log(`[OmbRouter]`);
  console.log(`[OmbRouter]   Derived EVM Address    : ${derived.evmAddress}`);
  if (solanaAddress) {
    console.log(`[OmbRouter]   Derived Solana Address : ${solanaAddress}`);
  }
  console.log(`[OmbRouter]`);
  console.log(`[OmbRouter]   If the Solana address above matches your funded`);
  console.log(`[OmbRouter]   wallet, recovery is safe to proceed.`);
  console.log(`[OmbRouter] ════════════════════════════════════════════════`);
  console.log(`[OmbRouter]`);
  ...
  console.log(`[OmbRouter] ✓ wallet.key restored at ${WALLET_FILE}`);
  console.log(`[OmbRouter]   Run: ombrouter (or: node dist/cli.js)`);
  console.log(`[OmbRouter]`);
}
...
export async function setupSolana(): Promise<{
  ...
  if (!savedKey) {
    throw new Error(
      "No EVM wallet found. Run OmbRouter first to generate a wallet before setting up Solana.",
    );
  }
  ...
  console.log(`[OmbRouter] Solana wallet set up successfully.`);
  console.log(`[OmbRouter] Mnemonic saved to ${MNEMONIC_FILE}`);
  console.log(`[OmbRouter] Existing EVM wallet unchanged.`);
...
export async function resolvePaymentChain(): Promise<"base" | "solana"> {
  const chainEnv = process["env"].OMBROUTER_PAYMENT_CHAIN;
  if (chainEnv === "solana") return "solana";
