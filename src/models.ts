...
export const BLOCKRUN_MODELS: BlockRunModel[] = [
  ...
  // OmbRouter maps free/xxx → nvidia/xxx before sending to BlockRun upstream.
...
/**
 * Build OpenClaw model list with per-model costs taken from merged pricing (e.g. cost_config overrides).
 */
export function buildOpenClawModelsWithPricing(modelPricing: Map<string, ModelPricing>): ModelDefinitionConfig[] {
  const primary: ModelDefinitionConfig[] = BLOCKRUN_MODELS.map((m) => {
  ...
  const aliasWithPricing: ModelDefinitionConfig[] = Object.entries(MODEL_ALIASES)
    .map(([alias, targetId]) => {
      const target = BLOCKRUN_MODELS.find((mod) => mod.id === targetId);
      if (!target) return null;
  ...
  return [...primary, ...aliasWithPricing];
}
