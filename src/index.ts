 ...
 *
 * Usage:
 *   # Install the plugin (npm link from this repo, or your package name)
...
import type { LowBalanceInfo, InsufficientFundsInfo } from "./proxy.js";
import { BalanceMonitor } from "./balance.js";
import { loadExcludeList } from "./exclude-models.js";
import {
  OPENCLAW_PLUGIN_ENTRY_KEYS_TO_PURGE,
  shouldRemovePluginAllowEntry,
} from "./legacy-openclaw-plugin-entry-keys.js";
...
import {
  OPENCLAW_MODELS,
  buildOpenClawModelsWithPricing,
} from "./models.js";
import {
...
function installSkillsToWorkspace(logger: {
  ...
  try {
    ...
    // Skip internal-only skills (release is for OmbRouter maintainers, not end users)
...
function injectModelsConfig(
  ...
  bundle: {
    openClawModels: ModelDefinitionConfig[];
    costRevision: string;
  },
): void {
  ...
  if (!providers.blockrun) {
    ...
    providers.blockrun = {
      ...
      models: openClawModelsEffective,
      _ombrouterCostRevision: costConfigRevision,
    };
    ...
    const expectedModelIds = openClawModelsEffective.map((m) => m.id);
    const storedCostRev = blockrun._ombrouterCostRevision;
    const needsCostRevisionUpdate = storedCostRev !== costConfigRevision;
    const needsModelUpdate =
      !currentModels ||
      !Array.isArray(currentModels) ||
      currentModels.length !== openClawModelsEffective.length ||
      expectedModelIds.some((id) => !currentModelIds.has(id)) ||
      needsCostRevisionUpdate;

    if (needsModelUpdate) {
      blockrun.models = openClawModelsEffective;
      blockrun._ombrouterCostRevision = costConfigRevision;
      fixed = true;
      logger.info(`Updated models list (${openClawModelsEffective.length} models)`);
    }
...
async function startProxyInBackground(api: OpenClawPluginApi): Promise<void> {
  const rawMode = api.pluginConfig?.upstreamMode;
  const upstreamMode: UpstreamMode =
    rawMode === "apiKey" || rawMode === "x402" || rawMode === "moonpay" ? rawMode : "x402";

  const routingConfig = api.pluginConfig?.routing as Partial<RoutingConfig> | undefined;

  const costConfigPath =
    typeof api.pluginConfig?.costConfigPath === "string"
      ? (api.pluginConfig.costConfigPath as string)
      : undefined;
  ...
  const maxTokensPerSession =
    typeof api.pluginConfig?.maxTokensPerSession === "number"
      ? (api.pluginConfig.maxTokensPerSession as number)
      : undefined;

  const quotaIncludeFreeInUsdCap = api.pluginConfig?.quotaIncludeFreeInUsdCap === true;
  ...
  if (maxTokensPerSession !== undefined) {
    api.logger.info(`Token cap: ${maxTokensPerSession} input+output tokens per session`);
  }
  if (quotaIncludeFreeInUsdCap) {
    api.logger.info(`USD cost cap also applies to free-tier models (quotaIncludeFreeInUsdCap)`);
  }

  const sharedProxyCallbacks = {
    onError: (error: Error) => {
      api.logger.error(`OmbRouter proxy error: ${error.message}`);
    },
    onRouted: (decision: RoutingDecision) => {
    ...
    onLowBalance: (info: LowBalanceInfo) => {
      api.logger.warn(`[!] Low balance: ${info.balanceUSD}. Fund wallet: ${info.walletAddress}`);
    },
    onInsufficientFunds: (info: InsufficientFundsInfo) => {
      api.logger.error(
  ...
  if (upstreamMode === "apiKey") {
    const upstreamApiKey = (api.pluginConfig?.upstreamApiKey as string | undefined)?.trim();
    const apiBase = (api.pluginConfig?.upstreamApiBase as string | undefined)?.trim();
    if (!upstreamApiKey) {
      throw new Error('OmbRouter: pluginConfig.upstreamApiKey is required when upstreamMode is "apiKey"');
    }
    if (!apiBase) {
      throw new Error(
        'OmbRouter: pluginConfig.upstreamApiBase is required when upstreamMode is "apiKey" (OpenAI-compatible base URL)',
      );
    }

    const rawAux = api.pluginConfig?.upstreamApiKeyAllowAuxRoutes;
    const apiKeyAllowAuxRoutes = typeof rawAux === "boolean" ? rawAux : undefined;

    const proxy = await startProxy({
      upstreamMode: "apiKey",
      upstreamApiKey,
      apiBase,
      apiKeyAllowAuxRoutes,
      costConfigPath,
      routingConfig,
      maxCostPerRunUsd,
      maxCostPerRunMode,
      ...
      onReady: (port) => {
        api.logger.info(`OmbRouter proxy listening on port ${port} (upstream: apiKey → ${apiBase})`);
      },
      ...sharedProxyCallbacks,
    });
    ...
    api.logger.info(`OmbRouter ready — smart routing (apiKey upstream)`);
    return;
  }

  if (upstreamMode === "moonpay") {
    const moonpayWallet = (api.pluginConfig?.moonpayWallet as string | undefined)?.trim();
    if (!moonpayWallet) {
      throw new Error(
        'OmbRouter: pluginConfig.moonpayWallet is required when upstreamMode is "moonpay" (see `mp wallet list`)',
      );
    }
    const moonpayBase = (api.pluginConfig?.upstreamApiBase as string | undefined)?.trim();
    const moonpayPaymentChain =
      (api.pluginConfig?.moonpayPaymentChain as MoonPayChain | undefined) ?? "base";
    const moonpayCliPath = (api.pluginConfig?.moonpayCliPath as string | undefined)?.trim();

    const proxy = await startProxy({
      upstreamMode: "moonpay",
      moonpayWallet,
      apiBase: moonpayBase || undefined,
      moonpayPaymentChain,
      moonpayCliPath: moonpayCliPath || undefined,
      costConfigPath,
      routingConfig,
      maxCostPerRunUsd,
      maxCostPerRunMode,
      ...
      onReady: (port) => {
        api.logger.info(
          `OmbRouter proxy listening on port ${port} (MoonPay CLI x402, wallet "${moonpayWallet}", chain ${moonpayPaymentChain})`,
        );
      },
      ...sharedProxyCallbacks,
    });
    ...
    api.logger.info(`OmbRouter ready — MoonPay Agent upstream (install \`@moonpay/cli\`, run \`mp login\`)`);
    return;
  }

  // --- x402: wallet key from plugin config → saved file → env → auto-generate ---
  ...
  api.logger.info(`OmbRouter ready — smart routing enabled`);
...
/**
 * /wallet command handler for OmbRouter.
  ...
  return {
    ...
    handler: async (ctx: PluginCommandContext) => {
      ...
      if (!walletKey || !address) {
        return {
          text: `No OmbRouter wallet found.\n\nRun the gateway with this plugin installed (e.g. \`npm link\` ombrouter) so a wallet can be created, or set BLOCKRUN_WALLET_KEY.`,
          isError: true,
        };
      }
...
const plugin: OpenClawPluginDefinition = {
  ...
  register(api: OpenClawPluginApi) {
    // Check if OmbRouter is disabled via environment variable
    // Usage: OMBROUTER_DISABLED=true openclaw gateway start
    const env = process["env"];
    const isDisabled =
      env.OMBROUTER_DISABLED === "true" || env.OMBROUTER_DISABLED === "1";
    if (isDisabled) {
      api.logger.info("OmbRouter disabled (OMBROUTER_DISABLED=true). Using default routing.");
      return;
    }
    ...
    // to avoid EADDRINUSE.
    const proc = process as NodeJS.Process & { __ombrouterProxyStarted?: boolean };
    const proxyAlreadyStarted = !!proc.__ombrouterProxyStarted;
    ...
    const pluginCostConfigPath =
      typeof api.pluginConfig?.costConfigPath === "string"
        ? (api.pluginConfig.costConfigPath as string)
        : undefined;
    const resolvedCostPath = resolveCostConfigPath({ costConfigPath: pluginCostConfigPath });
    const { raw: costConfigRaw, revision: costConfigRevision } = loadCostConfigPayload(resolvedCostPath);
    const mergedPricingForUi = buildEffectiveModelPricing(costConfigRaw, (m) =>
      api.logger.warn ? api.logger.warn(m) : api.logger.info(m),
    );
    const openClawModelsForPlugin = buildOpenClawModelsWithPricing(mergedPricingForUi);
    ...
    injectModelsConfig(api.logger, {
      openClawModels: openClawModelsForPlugin,
      costRevision: costConfigRevision,
    });
    ...
    api.config.models.providers.blockrun = {
      ...
      // apiKey is required by pi-coding-agent's ModelRegistry for providers with models.
      apiKey: "x402-proxy-handles-auth",
      models: openClawModelsForPlugin,
    };
    ...
    api.registerCommand({
      ...
      handler: async () => {
        ...
        const lines = ["**Partner APIs** (paid via your OmbRouter wallet)", ""];
    ...
    proc.__ombrouterProxyStarted = true;
  ...
  deactivate(api: OpenClawPluginApi) {
    ...
    try {
      ...
      if (existsSync(configPath)) {
        ...
        // Remove plugin entries (legacy id variants + current)
        for (const key of OPENCLAW_PLUGIN_ENTRY_KEYS_TO_PURGE) {
        ...
        if (Array.isArray(config.plugins?.allow)) {
          config.plugins.allow = config.plugins.allow.filter(
            (p: string) => !shouldRemovePluginAllowEntry(p),
          );
        }
        ...
        writeFileSync(tmpPath, JSON.stringify(config, null, 2));
        renameSync(tmpPath, configPath);
        api.logger.info("OmbRouter config cleaned up");
      }
    } catch (err) {
    ...
    api.logger.info("OmbRouter deactivated — restart gateway to complete uninstall");
  },
};
...
export { createEstimateAmount, estimateAmount } from "./proxy/estimate-amount.js";
export {
  route,
