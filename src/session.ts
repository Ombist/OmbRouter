...
export type SessionEntry = {
  ...
  /** Cumulative usage tokens (input + output) for maxTokensPerSession */
...
export class SessionStore {
  ...
  /**
   * Add token usage to a session (after a successful completion).
   */
  addSessionTokens(sessionId: string, inputDelta: number, outputDelta: number): void {
    let entry = this.sessions.get(sessionId);
    if (!entry) {
      const now = Date.now();
      entry = {
    ...
    entry.sessionInputTokens = (entry.sessionInputTokens ?? 0) + inputDelta;
    entry.sessionOutputTokens = (entry.sessionOutputTokens ?? 0) + outputDelta;
  }

  /** Total input+output tokens recorded for the session. */
  getSessionTokensTotal(sessionId: string): number {
    const entry = this.sessions.get(sessionId);
    if (!entry) return 0;
    return (entry.sessionInputTokens ?? 0) + (entry.sessionOutputTokens ?? 0);
  }
