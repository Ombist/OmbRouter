 ...
 * against OmbRouter's built dist/ to catch regressions like process.env
...
/** Resolve OmbRouter dist dir — relative to this file, fall back to Docker path. */
function resolveOmbRouterDist(): string {
  const local = resolve(__dirname, "../../dist");
  try {
    readdirSync(local);
    return local;
  } catch {
    return "/opt/ombrouter/dist";
  }
}
