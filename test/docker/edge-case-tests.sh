...
# OmbRouter Edge Case Test Suite
...
test_fresh_install() {
    ...
    npm install -g ombrouter 2>&1 | head -5

    if command -v ombrouter &> /dev/null; then
        log_pass "OmbRouter installed"
        ombrouter --version
    else
        log_fail "OmbRouter not in PATH"
        return 1
    fi
}
...
test_openclaw_plugin_install() {
    ...
    bash $(npm root -g)/ombrouter/scripts/reinstall.sh 2>&1 || {
        log_fail "Reinstall script failed"
        return 1
    }

    # Verify extension installed
    if [ -f "$HOME/.openclaw/extensions/ombrouter/dist/index.js" ]; then
...
test_invalid_model_handling() {
    ...
    local response
    response=$(curl -s -X POST "http://localhost:$OMBROUTER_PORT/v1/chat/completions" \
...
test_proxy_startup() {
    log_test "3.1" "Proxy startup on port $OMBROUTER_PORT"

    # Kill any existing proxy
    lsof -ti :$OMBROUTER_PORT | xargs kill -9 2>/dev/null || true
    sleep 1

    # Start proxy in background
    BLOCKRUN_WALLET_KEY="0x$(openssl rand -hex 32)" ombrouter &
    PROXY_PID=$!

    if wait_for_port $OMBROUTER_PORT 10; then
        log_pass "Proxy started on port $OMBROUTER_PORT (PID: $PROXY_PID)"
    else
...
test_proxy_health_check() {
    ...
    local response
    response=$(curl -s "http://localhost:$OMBROUTER_PORT/health" 2>/dev/null || echo "failed")
...
test_proxy_models_endpoint() {
    ...
    local response
    response=$(curl -s "http://localhost:$OMBROUTER_PORT/v1/models" 2>/dev/null || echo '{"error":"failed"}')
...
test_proxy_graceful_shutdown() {
    ...
    local pid=$(lsof -ti :$OMBROUTER_PORT 2>/dev/null || echo "")

    if [ -n "$pid" ]; then
        kill $pid 2>/dev/null
        sleep 2

        if ! nc -z localhost $OMBROUTER_PORT 2>/dev/null; then
...
test_port_conflict_handling() {
    ...
    nc -l -p $OMBROUTER_PORT &
    NC_PID=$!
    sleep 1

    # Try to start ombrouter (should fail or use different port)
    BLOCKRUN_WALLET_KEY="0x$(openssl rand -hex 32)" timeout 5 ombrouter 2>&1 | head -3 || true
...
test_wallet_generation() {
    ...
    rm -f ~/.ombrouter/wallet.json 2>/dev/null || true

    # Start proxy briefly to generate wallet
    BLOCKRUN_WALLET_KEY="" timeout 3 ombrouter 2>&1 | head -5 || true

    if [ -f ~/.ombrouter/wallet.json ]; then
        log_pass "Wallet auto-generated"
        jq -r '.address' ~/.ombrouter/wallet.json | head -1
    else
...
test_wallet_persistence() {
    ...
    get_wallet_from_health() {
        ...
        BLOCKRUN_WALLET_KEY="$key" ombrouter >/tmp/ombrouter-wallet-persistence.log 2>&1 &
        pid=$!
        ...
        response=$(curl -s "http://localhost:$OMBROUTER_PORT/health" 2>/dev/null || echo "")
...
test_openclaw_plugins_allow() {
    ...
    if [ -f "$CONFIG_FILE" ]; then
        if jq -e '.plugins.allow | index("ombrouter")' "$CONFIG_FILE" &>/dev/null; then
            log_pass "ombrouter in plugins.allow"
        else
            log_fail "ombrouter not in plugins.allow"
        fi
    else
        log_skip "openclaw.json not found"
    fi
...
main() {
    echo ""
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║       OmbRouter Edge Case Test Suite v1.0                   ║${NC}"
