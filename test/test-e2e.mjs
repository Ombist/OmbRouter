 ...
 * 2. Start OmbRouter proxy pointing to the mock server
...
async function runTests() {
  console.log("=== OmbRouter E2E Test: Thinking Token Stripping ===\n");
  ...
  // 2. Start OmbRouter proxy pointing to mock server
  let proxy;
  try {
    proxy = await startProxy({
      ...
      port: 0,
      onReady: (port) => console.log(`✓ OmbRouter proxy started on port ${port}`),
    });
  } catch (err) {
