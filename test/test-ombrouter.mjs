 ...
 *
 * Run: node test/test-ombrouter.mjs
 */
...
for (const m of OMBROUTER_MODELS) {
...
test("OMBROUTER_MODELS has 20+ models", () => {
  assertTrue(OMBROUTER_MODELS.length >= 20, `Only ${OMBROUTER_MODELS.length} models`);
});
...
test("All OMBROUTER_MODELS have required fields", () => {
  for (const model of OMBROUTER_MODELS) {
...
test("Model IDs are unique in OMBROUTER_MODELS", () => {
  const ids = new Set();
  for (const model of OMBROUTER_MODELS) {
