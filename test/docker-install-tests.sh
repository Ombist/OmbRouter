...
# OmbRouter Docker Installation Tests
...
test_fresh_install() {
    ...
    if command -v ombrouter &> /dev/null; then
        log_pass "OmbRouter installed successfully"
    else
        log_fail "OmbRouter command not found after installation"
        return 1
    fi

    VERSION=$(ombrouter --version 2>/dev/null || echo "")
    if [ -n "$VERSION" ]; then
...
test_uninstall() {
    ...
    if ! command -v ombrouter &> /dev/null; then
        log_pass "OmbRouter uninstalled successfully"
    else
        log_fail "OmbRouter command still available after uninstall"
        return 1
    fi
}
...
test_reinstall() {
    ...
    if command -v ombrouter &> /dev/null; then
        log_pass "OmbRouter reinstalled successfully"
    else
        log_fail "OmbRouter command not found after reinstall"
        return 1
    fi
}
...
test_openclaw_install() {
    ...
    PLUGIN_FILE="$HOME/.npm-global/lib/node_modules/ombrouter/openclaw.plugin.json"
...
test_upgrade() {
    ...
    npm uninstall -g ombrouter 2>/dev/null || true

    # Install old version
    npm install -g ombrouter@0.8.25

    OLD_VERSION=$(ombrouter --version 2>/dev/null || echo "")
    log_info "Installed version: $OLD_VERSION"

    # Upgrade to latest
    npm install -g ombrouter

    NEW_VERSION=$(ombrouter --version 2>/dev/null || echo "")
...
test_custom_wallet() {
    ...
    export OMBROUTER_WALLET_PRIVATE_KEY="$CUSTOM_KEY"

    # Verify installation with custom key works
    if command -v ombrouter &> /dev/null; then
        log_pass "OmbRouter installed with custom wallet key"
    else
        log_fail "Installation failed with custom wallet key"
        return 1
    fi

    unset OMBROUTER_WALLET_PRIVATE_KEY
}
...
test_package_files() {
    ...
    PKG_DIR="$HOME/.npm-global/lib/node_modules/ombrouter"
...
test_version_accuracy() {
    ...
    CLI_VERSION=$(ombrouter --version 2>/dev/null || echo "")
...
test_full_cleanup() {
    ...
    if ! command -v ombrouter &> /dev/null; then
        log_pass "OmbRouter command removed from PATH"
    else
        log_fail "OmbRouter command still in PATH"
        return 1
    fi
}
